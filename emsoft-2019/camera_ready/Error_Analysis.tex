...
$p$ and can also be called explicitly without the precision optimization. \Mahmoudd{To encourage reproducible research, our implementation is made available online.\footnote{\url{https://github.com/rospoly/rmpc-daisy}}}
\Mahmoudd{\begin{remark}
	Note that we use MATLAB and Daisy solely for offline computations. The output of our algorithm contains (almost) all the information required for implementation of the verified controller over embedded systems. Therefore, the same savings can be expected when the controller is implemented on an actual embedded system.
\end{remark}}
\begin{comment}
