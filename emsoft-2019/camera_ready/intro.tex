...
\Mahmoudd{We note that EMPC is a class of controllers that is mainly appropriate for implementation over processors with limited computational power and memory. Typical memory of these processors is in the order of tens of kilobytes (e.g. $32$ KB) and hence $21$ KB is considered as a significant saving. Also, the memory savings do not affect the run-time since  the controller synthesis and the corresponding mixed precision computations are all performed offline.}\\
