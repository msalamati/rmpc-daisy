...
However, there are still bottleneks in implementing explicit MPC on resource-constrained embedded micro-controllers.
...
implementation.
\eva{We bound all of the errors, including ones from choosing the wrong region. Worth mentioning here?}
\eva{According to sec 5, the loop is not due to mixed-tuning failing, but due to the region error being too large}
