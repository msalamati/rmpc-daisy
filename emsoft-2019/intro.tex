...
Our implementation finds that our mixed-precision implementation can save up to 20\% memory (on average, \RM{XXX}\%) in controller
implementations over a uniform-precision implementation (and a saving of \RM{XXX}\% over a uniform 32-bit implementation), while
maintaining the correctness of the controller.
In absolute terms, the saving corresponds to \RM{YYY} KB over a uniform precision implementation on our largest benchmark.
