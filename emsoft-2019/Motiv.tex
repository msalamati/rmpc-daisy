...
\centering
	...
	\tikzstyle{pinstyle} = [pin edge={to-,thin,black}]
	{
		...
		\node [block3,below=1.3cm of RMPC,
		...
		node distance=1cm] (delta) {Increase $\Delta$};
		\node [block1,below = 0.8cm of done] (output) {Low-precision controller};
	...
	%\end{center}
	%\end{adjustbox}
	\caption{Overview of the proposed memory-efficient robust MPC control design.}
	\label{fig:overview}
\end{figure}
...
\RM{the following setence is not understandable. This is the first time we introduce incorrect region selection.
Who is Daisy?}
\RM{A better version is to informally define incorrect region selection. Say that we run an algorithm for incorrect region
selection to bound the possible error. 
Break the sentence into two. Introduce Daisy as a mixed-precision tuning tool.}

The output of robust  EMPC given by Matlab decomposes the control domain into a finite number of polyhedra together with an affine map for each domain. At each time step, the state vector $x_k$ is read (or estimated) and based on the domain that it belongs to, the output of controller is determined. However, due to error by fixed-point implementation, it is possible that the domain is selected incorrectly. This causes an error term which we refer to it as incorrect region selection error.
Over the high-precision controller, we compute the error due to an incorrect region selection $e$. In case that $e<\Delta_0$, we would like to find the smallest number of bits required for implementing the controller such that we are still within the safe bound $\Delta_0$. To do so, we ask Daisy, which is a mixed-precision tuning tool, to provide us with a mixed-precision scheme such that the associated implementation error is at most $\Delta_0$, that is,
...
Unfortunately, $\Delta_0=0.01$ is not realizable, as the region selection error is $0.019$ and hence $e>\Delta_0$. 
...
The controller has $14$ regions. Following demostrates one sample mapping, including the activation condition and its corresponding affine function
\begin{equation*}
\resizebox{.9\hsize}{!}{$
\begin{bmatrix}
-0.005  &0.999\\
...
\end{equation*}
where, output of robust EMPC at state $x_k$ is denoted by $u(x_k)$. Notice that all the rest of mappings have similar format.
