...
As a simple example, consider the problem of designing a controller for an inverted pendulum depicted in Fig.~\ref{fig:inverted_pendulum}~{(a)}.
...
The disturbance $w_k\in\mathbb R^2$ bounded with $\|w_k\|\leq \Delta$ captures the modeling error due to 
...
(i) design a robust MPC that achieves the performance objectives including hard constraints in spite of a bounded
distubrance $\Delta$ modeling the implementation error due to fixed precision;
...
The mixed-precision scheme given by Daisy results in an implementation with $6084$ bits in all 
