...
(i) design a robust MPC controller that achieves the performance objectives including hard constraints in spite of \Sadegh{an additional bounded
...
\end{equation}
 ...
 \Sadegh{
For our example, we choose $\Delta_0=0$ and $\mathcal W = \{0\}$. 
...
\Sadegh{
Unfortunately, $\Delta_0=0$ is not realizable, as the implementation error is always greater than zero either due to selecting the wrong region in the affine controller function or due to the finite-precision computation of the control action.
%\eva{Is this the error due to mixed-precision or due to wrong regions?}
Therefore, we define $\Delta_1 := \Delta_0 + \delta$ with $\delta$ being a fixed threshold (we chose $\delta = 0.05$) and enlarge the disturbance set as $\mathcal W_{\Delta_1}$. 
