...
\centering
	...
	\node [block,scale=1,text width=3.8cm, pin={[pinstyle]above:$\Delta=\Delta_0$},
	...
	\node [block3, scale=1,below=1.3cm of RMPC,
	node distance=3.8cm,text width=4.4cm,  label={[anchor=south east, draw=black, inner sep=1pt]south east:Daisy}] (Uij) {Point location error computation};
	\node [draw, diamond, minimum height=3em, minimum width=3em, below =0.8cm of Uij, node distance=5cm] (decide) {$e\leq  \Delta$};
	\node [block3, below=0.8cm of decide,node distance=3cm, label={[anchor=south east, draw=black, inner sep=1pt]south east:Daisy}] (done) {Mixed-precision implementation};
