...
instance $\varepsilon_Q$ is an input), and should reference the lines in the
...
\eva{\autoref{lst:alg} needs another input: state-space model which goes into the MPC designer}
\eva{Add here the explanation why $\varepsilon_Q$ is an input}
From line 1 to 3, the procedure waits for input parameters from the designer: $\delta$ and $\varepsilon_{Q}$. $\delta$ is the maximal disturbance coefficient the controller has to be designed for. It is a problem dependent parameter, and thus can be any positive number. Typically, it is in the order of 0.1.
...
Once input parameters are verified, in line 7 we design the controller in MATLAB MPC Toolbox. The suite receives in input the coefficient $\delta$, and the state space model for the plant (e.g. inverted pendulum or aircraft) we need to control. The suite returns a piecewise affine function (F and G) together with a polytopes region partitioning (H and K).
