...
%states that two regions share at least a point, $\inputm{i}$ is the control action
%for controller $i$, and $err(\inputm{i})_{p}$ captures the approximation error for
%computing the control action $\inputm{i}$ in fixed precision $p$.
...
%control action $\inputm{i}$, but due to uncertainties we actually choose control action $\inputm{j}$:
%\begin{align} \label{eq:maximization}
 % \max_{\forall i,j.\mathit{neighbor}(\mathcal{R}_{i},\mathcal{R}_{j})}|\inputm{i}-\inputm{j}|&= \\
 % \max_{\forall i,j.\mathit{neighbor}(\mathcal{R}_{i},\mathcal{R}_{j})}&|F_{i}\statevar+G_{i} - (F_{j}\statevar+G_{j})|\nonumber
%\end{align}

%Because of the linearity of the function $\inputm{i}-\inputm{j}$, and because of the
...
% for max error $err(\inputm{i})$ from (8). Then we give the errors and the vectors F,G
