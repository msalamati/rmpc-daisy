...
\Mahmoud{For the inverted pendulum problem depicted in Section \ref{sec:example}, we set  $m=.344$ kg, $b=0.48$ N s/m, $L=1.703$ m and $T_s=0.1$ s. The gain $K$ is selected such the $\bar A$ has poles at $-0.1$ and $-0.5$. Moreover, we select $N=2$, $R=1$ and $Q=Q_F=100\mathbf{\it I}$, where $\mathbf{ \it I}$ denotes the identity matrix of proper size. The aircraft 4-D model is borrowed from \cite{Kapasouris:1998}. Using the gain $K$, the poles of $\bar A$ are placed at $-5$, $-3$, $-1$ and $-2$. The robust MPC problem is solved for $N=2$, $R=\mathbf{\it I}$ and $Q=Q_F=5\mathbf{\it I}$. Note that for both of the examples, matrices $R$, $Q$ and $Q_F$ are selected such that convergrnce to the origin is given more weight comparing to the control effort as long as constraints are satisfied. \\
...
\Mahmoud{The goal of experiments in this section is to show that our algorithm works well even for the case that the controller consists of thousands of PWA functions and regions. 
