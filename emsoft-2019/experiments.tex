...
needs to design a new controller (corresponding to the \texttt{else} branch in~\autoref{fig:overview}). 
The loop converges after 5 iterations (\texttt{then}
branch of the conditional) with $\Delta=0.112$. In each iteration, the value
...
We observe that our analysis scales up to 1200 controllers and 25000 hyperplanes \todo{these are not in the table!}
in a few hours.  The computed minimal uniform precision saves on average $49.8\%$ of memory
